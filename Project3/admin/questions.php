...
<body>
	...
	<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
		...
		<div class="container1 clearfix">
			<!-- Search questions -->
			<div>
				<form action = "search.php" method="get" style="float: left; width: 500px !important">
					...
					for($i = 1; $i <= $num; $i++){
						...
						for($y = 0; $y < sizeof($typeID); $y ++){
							if($type == 4){
								...
								SELECT label, value 
								FROM " . $typeTable . " 
								WHERE ans = 1 AND ansID = ".$typeID[$y];
								$result = $conn->query($sql) or die($conn->error);
								while($row = $result->fetch_assoc()) {
						...
						$sql = "
						SELECT question.score AS score, status.name AS status
						...
						echo '<div class="btn-group"><a class="btn btn-primary" href="edit.php?edit='.$i.'">Edit</a>';	
						echo '<br/>';
						echo "<a class='btn btn-warning' onClick=\"javascript: return confirm('Are you sure you want to inactivate this question?');\" href='delete.php?delete=".$i."'>Delete</a>
						</div>";			
						echo '</td></tr>';						
					}
					?>										
